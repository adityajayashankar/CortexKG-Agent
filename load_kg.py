    ...
    for i, row in enumerate(records, start=1):
        ...
        s.run(
            ...
            SET v.cve_id              = CASE WHEN $vid STARTS WITH 'CVE-'  THEN $vid ELSE v.cve_id END,
                ...
                v.vulnerability_name  = coalesce(v.vulnerability_name, $name),
                v.description         = coalesce(v.description, $desc),
                v.cvss_score          = coalesce(v.cvss_score, $cvss),
                v.epss_score          = coalesce(v.epss_score, $epss),
                v.confirmed_exploited = coalesce(v.confirmed_exploited, $kev),
                v.risk_level          = coalesce(v.risk_level, $risk)
            """,
            vid=vid,
        ...
        for sw in (row.get("affected_software") or []):
            ...
            s.run(
                """
                MERGE (p:Software {software_key: $sw})
                SET p.name = $sw
                MERGE (v:Vulnerability {vuln_id: $vid})
...
if cluster_map:
    with driver.session() as s:
        for cluster_id, cwes in cluster_map.items():
            s.run("MERGE (c:CWECluster {cluster_id: $cluster_id})", cluster_id=cluster_id)
            for cwe in sorted(cwes):
                s.run(
                    """
                    MERGE (w:CWE {cwe_id: $cwe})
                    MERGE (c:CWECluster {cluster_id: $cluster_id})
    ...
    with driver.session() as s:
        for row in corr_data:
            ...
            for rel in (row.get("related_vulnerabilities") or []):
                ...
                s.run(
                    """
                    MERGE (a:Vulnerability {vuln_id: $a})
                    MERGE (b:Vulnerability {vuln_id: $b})
                    MERGE (a)-[r:CORRELATED_WITH]->(b)
                    ON CREATE SET r.max_score = $score,
                                  r.signals   = $signals
                    ON MATCH SET  r.max_score = CASE WHEN $score > r.max_score
    ...
    with driver.session() as s:
        for pair in pairs:
            ...
            s.run(
                ...
                ON CREATE SET r.max_confidence = $conf,
                              r.sources        = [$src],
                              r.reasons        = [$reason]
                ON MATCH SET  r.max_confidence = CASE WHEN $conf > r.max_confidence
                                                 THEN $conf ELSE r.max_confidence END
                """,
                a=a,
                b=b,
